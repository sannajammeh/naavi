...
- [x] 1.2 Remove old source files (`src/NavigationMenu.tsx`, `src/NavigationMenuItem.tsx`) and clear existing `src/types.ts` and `src/index.ts`
...
- [x] 5.4 Implement `Trigger` — renders `<a>` by default via `useRender`, sets `role="menuitem"`, `aria-haspopup="true"`, `aria-expanded`, `aria-controls`, `data-navigation-menu-trigger`, `data-value`; handles click toggle, mouseenter/mouseleave for hover
- [x] 5.5 Implement `Content` — renders `<ul role="menu">` via `useRender`, sets `data-state`, `data-navigation-menu-content`, `data-value`, `aria-label`, `aria-labelledby`; provides `DepthContext` with incremented depth; portals into Viewport if available; handles mouseenter/mouseleave for hover delay
- [x] 5.6 Implement `Link` — renders `<a>` by default via `useRender`, sets `role="menuitem"`, `data-navigation-menu-link`; handles click to close menus (respects `closeOnClick`)
...
- [x] 7.1 Implement `Viewport` — renders `<div data-navigation-menu-viewport>`, registers element via `RootContext.setViewport`, sets `data-state` based on any open content
...
- [x] 9.2 Add playground CSS — style targeting `data-navigation-menu-*` and `data-state` attributes to control visibility, positioning, and focus styling
