...
Portal SHALL render its children into a target DOM element using `createPortal`. If a `selector` prop is provided, Portal SHALL use the matching DOM element. If no selector is provided, Portal SHALL create a `<div data-naavi-portal>` appended to `document.body`.
...
- **THEN** a `<div data-naavi-portal>` SHALL be created in `document.body` and children SHALL be rendered inside it

#### Scenario: Portal cleanup
- **WHEN** Portal unmounts and it created its own container (no selector)
- **THEN** the created `<div data-naavi-portal>` SHALL be removed from `document.body`

### Requirement: Viewport as optional Content target
Viewport SHALL render a `<div data-naavi-viewport>` element. When Viewport is present in the component tree, Content components SHALL portal their `<ul role="menu">` into the Viewport element instead of rendering in-place.
